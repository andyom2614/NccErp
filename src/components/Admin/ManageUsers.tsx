...
import { Users, Plus, CreditCard as Edit2, Trash2, X, Mail, Lock, User, Shield, Search, Building, MapPin } from 'lucide-react';
...
export const ManageUsers = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation logic
    if (formData.role === 'ano') {
      if (!formData.college.trim()) {
        setError('College is required for ANO role');
        setLoading(false);
        return;
      }
      if (!formData.unit.trim()) {
        setError('Unit is required for ANO role');
        setLoading(false);
        return;
      }
    } else if (formData.role === 'clerk' || formData.role === 'co') {
      if (!formData.unit.trim()) {
    ...
    try {
      if (editingUser) {
        ...
        const updateData = {
          ...
          role: formData.role,
          unit: formData.unit,
          college: formData.role === 'ano' ? formData.college : null,
        };
        
        await updateDoc(doc(db, 'users', editingUser.id), updateData);
        setSuccess('User updated successfully!');
      } else {
        // Create new user using secondary app to avoid logging out current admin
        ...
        try {
          ...
          // Use the UID as the document ID and store user data
          const userData = {
            name: formData.name,
            email: formData.email,
            role: formData.role,
  ...
  const handleCloseModal = () => {
    setShowModal(false);
    setEditingUser(null);
    setFormData({ name: '', email: '', password: '', role: 'ano', college: '', unit: '' });
    setError('');
  };
      ...
      >
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by name, email, role, college, or unit..."
              value={searchQuery}
        ...
        {showModal && (
          ...
          >
            ...
            >
              ...
              <form id="userForm" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-3">
                <div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        required={formData.role === 'ano'}
                      />
                    </div>
                    <p className="text-xs text-gray-500 mt-1">Required for ANO role</p>
                  </div>
                )}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">Required for all roles</p>
                </div>
